...
class VoyagerBreadWalletsController extends VoyagerController
{
    ...
    public function index(Request $request)
    {
        ...
        $walletDataSum = Wallet::where('typer', 'deposit')->sum('amount');
        $walletData['total_money'] = $walletDataSum;

        $walletDataWin = Wallet::where('typer', 'winning')->sum('amount');
        $walletDataOut = Wallet::where('typer', 'win-loss-out')->sum('amount');
        $walletDataDraw = Wallet::where('typer', 'win-loss-draw')->sum('amount');
        $walletDataTransOut = Wallet::where('typer', 'out-transfer-deposit')->sum('amount');
        $walletDataWith = $walletDataWin - ($walletDataOut + $walletDataDraw + $walletDataTransOut);
        $walletData['total_withdrawable'] = $walletDataWith;

        $walletData['total_credit'] = $walletDataSum - $walletDataWith;

        $walletData['total_won'] = $walletDataWith;

        $walletDataLoss = Wallet::where('typer', 'losing')->sum('amount');
        $walletData['total_loss'] = $walletDataLoss;

        $walletData['total_commission'] = number_format($walletDataWith/0.95 - $walletDataWith, 2, '.', '');

        $withdraw_data = Wallet::where('typer','win-loss-draw')->where('notes','withdraw-pending')->orWhere('notes','withdraw-block')->orderBy('id', 'desc')->paginate(15);
