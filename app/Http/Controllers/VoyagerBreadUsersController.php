...
class VoyagerBreadUsersController extends VoyagerController
{
    ...
    public function index(Request $request)
    {
        ...
        $userDataCount = User::where('id','!=',0 )->count();
        $userData['total_users'] = $userDataCount;
        $userDataOnlines = User::where('online_status','online' )->get();

        $userData['total_online'] = 0;
        foreach($userDataOnlines as $userDataOnline){
            if (abs(strtotime(date('Y-m-d H:i:s')) - strtotime($userDataOnline->updated_at)) < 20) {
                $userData['total_online'] += 1;
            }
        }
        $userDataOfflines = User::where('online_status','offline' )->get();
        $userData['total_offline'] = 0;
        foreach($userDataOfflines as $userDataOffline){
            if (abs(strtotime(date('Y-m-d H:i:s')) - strtotime($userDataOffline->updated_at)) > 20) {
                $userData['total_offline'] += 1;
            }
        }
        $userDataOfflinex = User::where('online_status',null )->count();
        $userData['total_offline'] += $userDataOfflinex;
        $userDataActive = User::where('email_verified_at','!=',null )->count();
        $userData['total_activated'] = $userDataActive;
        $userDataNotActive = User::where('email_verified_at',null )->count();
        $userData['total_non_activated'] = $userDataNotActive;
